...
func detect_feedsource_type(filepath string) uint {
	f, err := os.Open(filepath)
	if err != nil {
	...
	err = decoder.Decode(&v)
	return _source_types[v.XMLName.Local]
}
...
func (this rss_channel) to_source() FeedSource {
	v := FeedSource{
		...
		Local:       "", // filled later
		Period:      this.ttl(),
		DueAt:       unixtime_nano_rfc822(this.LastBuildDate) + int64(this.ttl())*int64(time.Hour),
